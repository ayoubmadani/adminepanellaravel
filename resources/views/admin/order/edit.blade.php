...
<form action="{{ route('orders.update', $order->id) }}" method="POST">
        ...
        @foreach($order->items as $index => $item)
            <div class="item-row grid grid-cols-1 md:grid-cols-6 gap-4 mb-4" data-index="{{ $index }}">
                <div>
                    ...
                    <input type="hidden" class="product-id" name="products[{{ $index }}][product_id]" value="{{ $item->product_id }}">
                </div>
                <div>
                    <label class="text-sm text-gray-700">Product</label>
                    <input type="text" class="product-name w-full px-3 py-2 border rounded-md bg-gray-100" value="{{ $item->product->name ?? '' }}" readonly>
                </div>
                <div>
                    <label class="text-sm text-gray-700">Price</label>
                    <input type="text" class="price-input w-full px-3 py-2 border rounded-md bg-gray-100" name="products[{{ $index }}][price]" value="{{ $item->price }}" readonly>
                </div>
                <div>
                    <label class="text-sm text-gray-700">Quantity</label>
                    <input type="number" class="quantity-input w-full px-3 py-2 border rounded-md" name="products[{{ $index }}][quantity]" value="{{ $item->quantity }}" min="1">
                </div>
                <div>
    ...
    <div class="mt-6 text-right">
        <button type="submit" class="px-6 py-2 bg-green-600 text-white rounded hover:bg-green-700 transition">Update Order</button>
    </div>
</form>
