...
@section('content')
    <script defer>
        ...
        function updateIndexes() {
            document.querySelectorAll('.item-row').forEach((row, index) => {
                ...
                row.querySelector('.product-name').name = `items[${index}][name]`;
                row.querySelector('.price-input').name = `items[${index}][price]`;
    ...
    <h1 class="text-2xl font-bold text-gray-800 mb-6">إنشاء طلب جديد</h1>
    ...
    <form action="{{ route('orders.store') }}" method="POST">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
            <div class="bg-white p-4 rounded-lg border">
                <h2 class="font-semibold text-lg mb-4">معلومات العميل</h2>
                <div class="space-y-2">
        ...
        <div id="items-container">
            <div class="item-row grid grid-cols-1 md:grid-cols-6 gap-4 mb-4" data-index="0">
                ...
                <div>
                    ...
                    <input type="text" class="price-input w-full px-3 py-2 border rounded-md bg-gray-100" readonly
                        name="items[0][price]">
                </div>
                <div>
                    <label class="text-sm text-gray-700">Quantity</label>
                    <input type="number" class="quantity-input w-full px-3 py-2 border rounded-md" value="1"
                        min="1" name="items[0][quantity]">
                </div>
                <div>
